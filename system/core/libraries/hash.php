...
class hash {
    ...
    /* CUSTOM decription 
    */
    public function decryptme($string){
        $x = explode('/', $string);
        $this->hash_encryption($x[0]);
        return $this->decrypt($x[1]);
    }
    /**
    ...
    function encryptMe_($string){
        $var = random_string();
        $this->hash_encryption($var);
        $encrypted = $this->encrypt($string);
        return $var."::".$encrypted;
    }

    function decryptMe_($string){
        $string = str_replace(';', '/', $string);
        $d = explode("::", $string);
        $this->hash_encryption($d[0]);
        $encrypted = $this->decrypt($d[1]);
        return $encrypted;
    }
